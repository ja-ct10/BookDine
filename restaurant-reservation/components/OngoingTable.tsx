...
import { updateReservationStatuses } from '@/lib/mockData';
...
/**
 * OngoingTable Component
 * 
 * Displays currently active reservations (Status='Arrived') with:
 * - Status change functionality via dropdown
 ...
 * - Toast notifications for automatic status changes
 * 
 * Requirements: 6.2, 6.3, 6.4, 6.5, 10.2, 10.3, 10.4
 */
const OngoingTable: React.FC<OngoingTableProps> = ({
  reservations,
  onRefresh,
}) => {
  ...
  // Real-time updates every 2 seconds with automatic status updates
  useEffect(() => {
    const interval = setInterval(async () => {
      // First, update statuses automatically (Arrived → Completed when time passes)
      const result = await updateReservationStatuses();
      
      // Show toast notification if any reservations were automatically completed
      if (result.updated > 0) {
        toast.info(
          `${result.updated} reservation${result.updated > 1 ? 's' : ''} automatically completed`,
          5000
        );
      }
            ...
            {sortedReservations.length === 0 ? (
              <tr>
                <td colSpan={9} className="px-4 py-8 text-center text-gray-500">
                  {searchQuery ? 'No ongoing reservations found matching your search' : 'No ongoing reservations'}
                </td>
              </tr>
            ) : (
